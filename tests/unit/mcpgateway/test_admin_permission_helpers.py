...
class TestValidateSectionPermissions:
    ...
    def test_validate_section_permissions_with_mismatches(self, mock_logger):
        """Test validate_section_permissions with mismatches (line 332)."""
        ...
        # Should log warnings about mismatches
        assert mock_logger.warning.call_count >= 2
        warning_calls = [call[0][0] for call in mock_logger.warning.call_args_list]
        assert any("mismatches" in call for call in warning_calls)
        assert any("tools" in call for call in warning_calls)
